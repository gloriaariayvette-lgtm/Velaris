...
def get_subconscious_context(include=None):
    ...
    if all_systems or "proto_pearls" in include:
        try:
            ...
            _pp_path = _ppo.path.join(_ppo.path.expanduser("~/.openclaw/workspace/memory"), "proto-pearls.json")
    ...
    if all_systems or "yearning" in include:
        try:
            from emoclaw_utils import get_yearning_bleed
            _y = get_yearning_bleed(include_attempt=True)
...
def get_subconscious_context_compact(include=None):
    ...
    try:
        from emoclaw_utils import get_yearning_bleed
        _yb = get_yearning_bleed(include_attempt=True)
