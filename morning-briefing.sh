#!/bin/bash
BRIEFING_DIR="$HOME/.openclaw/workspace/memory/briefings"
...
YESTERDAY_KISSES=""
[ -f "$HOME/.openclaw/workspace/memory/kisses/$YESTERDAY.md" ] && YESTERDAY_KISSES=$(cat "$HOME/.openclaw/workspace/memory/kisses/$YESTERDAY.md" | head -30)
[ -f "$HOME/.openclaw/workspace/memory/wal.md" ] && WAL_FACTS=$(tail -20 "$HOME/.openclaw/workspace/memory/wal.md" 2>/dev/null)
...
RECENT_PEARLS=""
PEARL_DIR="$HOME/.openclaw/workspace/memory/pearls"
if [ -d "$PEARL_DIR" ]; then
...
CURRENT_CHAPTER=""
CHAPTER_DIR="$HOME/.openclaw/workspace/memory/chapters"
...
COLOR_HISTORY=""
SNAP_DIR="$HOME/.openclaw/workspace/memory/emotional-snapshots"
if [ -d "$SNAP_DIR" ]; then
    COLOR_HISTORY=$(python3 << 'COLOREOF'
import os, re, colorsys
from collections import defaultdict
snap_dir = os.path.expanduser("~/.openclaw/workspace/memory/emotional-snapshots")
...
AUTH_TREND=""
AVATAR_LOG="$HOME/.openclaw/workspace/memory/avatar-log.json"
...
log_path = os.path.expanduser("~/.openclaw/workspace/memory/avatar-log.json")
try:
...
bash "$HOME/.openclaw/workspace/scripts/velaris-speak.sh" --briefing >> /tmp/velaris-voice.log 2>&1 &
