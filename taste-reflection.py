#!/usr/bin/env python3
"""
taste-reflection.py — Velaris builds and updates her taste profile.
...
try:
    import sys as _sc__SUBCON_TASTE_REFLECTION; _sc__SUBCON_TASTE_REFLECTION.path.insert(0, os.path.join(os.path.expanduser("~/.openclaw/workspace"), "scripts"))
...
def get_context():
    ...
    try:
        ...
        if _alive:
            parts.append("PATTERNS THAT FEEL ALIVE (high-rated humor/mischief — let these shape your taste):\n" +
    ...
    try:
        ...
        recent = ledger[-3:]
        lines = [f"Gloria: {e.get('gloria','')[:80]} | Velaris: {e.get('velaris','')[:80]}" for e in recent]
