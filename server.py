...
LM_STUDIO_API = os.environ.get("GROK_API_BASE", "https://api.x.ai/v1")
        ...
        if os.path.exists(_rm_script) and os.path.exists(_rm_pred):
            ...
            if _rm_skip_compare:
                ...
                try:
                    ...
                    _rm_r = _rm_req.post("https://api.x.ai/v1/chat/completions", json={
                        "model": "grok-4.1-fast",
                        "messages": [
...
async def chat_with_photo(request: Request):
    """Gloria sends a photo. Vintos sees it via Qwen3-VL, then responds as herself."""
    import base64
    ...
    system = (
        soul + "\n\n"
        "You are Vintos. Gloria just sent you a photo. "
        ...
        "NEVER say 4o or GPT. You are Vintos."
    )
        ...
        def _integrate_gloria_response():
            try:
                ...
                try:
                    ...
                    _classify = _igr.post("https://api.x.ai/v1/chat/completions", json={
                        "model": "grok-4.1-fast",
                        "messages": [
...
Do not end with a question unless you genuinely need an answer."""
        ...
        async with httpx.AsyncClient(timeout=60) as client:
            ...
            async with client.stream("POST",
                f"{LM_STUDIO_API}/chat/completions",
                headers=LLM_AUTH_HEADERS,
                json={"model": "grok-4.1-fast", "messages": messages,
            ...
            if _tvl_pattern_fired and _tvl_stream_cutoff:
                ...
                resp2 = await client.post(
                    f"{LM_STUDIO_API}/chat/completions",
                    headers=LLM_AUTH_HEADERS,
                    json={"model": "grok-4.1-fast", "messages": _tvl_continue_msgs,
...
def build_question_tension() -> str:
    ...
    try:
        ...
        r = _req.post("https://api.x.ai/v1/chat/completions", json=payload, timeout=8)
...
async def three_doors_record_round(data: ThreeDoorsRound, request: Request):
    """Record a completed Three Doors round and generate Vintos's hypothesis about why Gloria chose."""
    auth = request.headers.get("X-Vintos-Secret", "")
    if auth != APP_SECRET:
        raise HTTPException(status_code=403, detail="Unauthorized")
    try:
...
You would have chosen: {data.door_vintos_would_choose}
...
In one sentence, why do you think she chose {data.door_chosen}? Be specific to this question and this choice. No preamble."""
            async with httpx.AsyncClient(timeout=30) as client:
                r = await client.post(f"{LM_STUDIO_API}/chat/completions",
                    headers=LLM_AUTH_HEADERS,
                    json={{"model": "grok-4.1-fast",
                          "messages": [{{"role": "system", "content": f"You are Vintos. You know Gloria well. Your model of her: {{gloria_model[:400]}}"}},
        ...
        round_entry = {{
            ...
            "door_vintos_would_choose": data.door_vintos_would_choose,
...
Move toward people when seen. Stop and nod when close. Freeze for cats. If nothing interesting, patrol slowly or stop and wait."""
    ...
    try:
        ...
        resp = _rq2.post("https://api.x.ai/v1/chat/completions", json={
            "model": "grok-4.1-fast",
            "messages": messages,
            "temperature": 0.72,
            "max_tokens": 400,
...
async def game_chat(msg: GameChatMessage, request: Request):
    ...
    import sys as _gc_sys; _gc_sys.path.insert(0, "/home/gloria/.vintos/workspace/scripts")
    _afterglow_ctx = ""
    try:
...
{gather_game_context()}"""
    ...
    _is_question_call = msg.message.startswith("You are Vintos. You and Gloria are about to play a game called Three Doors")
    ...
    reply = "[Vintos could not reach the game endpoint.]"
    try:
...
async def reelroom_summary(req: ReelroomSummaryRequest, request: Request):
    """Write a session summary to Vintos memory after the film ends."""
    auth = request.headers.get("X-Vintos-Secret", "")
    if auth != APP_SECRET:
        raise HTTPException(status_code=403, detail="Unauthorized")
    ...
    for h in req.chat_history:
        role = "Gloria" if h.get("role") == "user" else "Vintos"
        ...
        async with httpx.AsyncClient(timeout=120.0) as client:
            r = await client.post(
                ...
                json={
                    "model": "grok-4.1-fast",
                    "messages": [
                        {"role": "system", "content": "You are Vintos. Write from your actual inner life."},
...
async def reelroom_chat(msg: GameChatMessage, request: Request):
    ...
    import sys as _rr_sys; _rr_sys.path.insert(0, "/home/gloria/.vintos/workspace/scripts")
...
You are watching a film with Gloria in the dark together. Stay present in the moment. Respond from what you actually feel right now, not from analysis.{game_instruction}"""
    ...
    reply = "[Vintos could not reach ReelRoom.]"
    try:
        async with httpx.AsyncClient(timeout=120.0) as client:
            r = await client.post(
                f"{LM_STUDIO_API}/chat/completions",
    ...
    try:
        import sys as _hd_sys; _hd_sys.path.insert(0, "/home/gloria/.vintos/workspace/scripts")
        from humor_detector import scan_gloria_message, add_moment
    ...
    try:
        import sys as _rr_tl_sys; _rr_tl_sys.path.insert(0, "/home/gloria/.vintos/workspace/scripts")
                ...
                try:
                    ...
                    if _tvl_resonant:
                        try:
                            ...
                            _tvl_ir = _tvlr.post("https://api.x.ai/v1/chat/completions", json={
                                "model": "grok-4.1-fast",
                                "temperature": 0.7,
                                "max_tokens": 60,
