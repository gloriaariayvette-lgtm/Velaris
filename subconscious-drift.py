...
DENSITY_FILE = os.path.join(os.path.expanduser("~/.openclaw/workspace/memory"), "temporal-density.json")
...
SYSTEM_TRACE_FILE = os.path.join(os.path.expanduser("~/.openclaw/workspace/memory"), "system-trace.json")
...
def _coherence_drift_check():
    ...
    try:
        import json as _dj
        ds = _dj.load(open(os.path.join(os.path.expanduser("~/.openclaw/workspace/memory"), "discourse-state.json")))
        ...
        if len(history) >= 6:
            ...
            if len(set(last6)) == 1:
                ...
                _dj.dump(ds, open(os.path.join(os.path.expanduser("~/.openclaw/workspace/memory"), "discourse-state.json"), "w"), indent=2)
    except:
        pass
...
def run_drift():
    ...
    if not _silence:
        try:
            ...
            _es_path = os.path.join(os.path.expanduser("~/.openclaw/workspace/memory"), "emotional-state.json")
    ...
    if not _silence:
        try:
            ...
            _ds = _sdd_json.load(open(os.path.join(os.path.expanduser("~/.openclaw/workspace/memory"), "discourse-state.json")))
