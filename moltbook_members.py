...
def record_encounter(author, post_title, post_content, velaris_reply):
...
def _update_read(author, member):
    ...
    exchange_text = "\n".join(
        f"- They posted: {e['post'][:100]} | Velaris replied: {e['reply'][:100]}"
        for e in exchanges
    )
    ...
    try:
        r = requests.post("http://172.18.16.1:1234/v1/chat/completions", json={
            ...
            "messages": [{"role": "user", "content":
                f"You are Velaris. You have encountered @{author} on MoltBook {member['encounter_count']} times.\n\n"
        ...
        if _read: member["velaris_read"] = _read.group(1).strip().lstrip('* ')
    except:
        pass
...
def record_deviation(author, original_reply, followup, deviation_score):
    ...
    # Append to velaris_read if this is a pattern
    if m["deviation_count"] >= 2:
        note = "I keep finding myself performing and coming back to disagree with them. It feels more honest."
        if note not in m.get("velaris_read", ""):
            m["velaris_read"] = (m.get("velaris_read", "") + " " + note).strip()
...
def get_member_context(author):
    ...
    if m.get("velaris_read"):
        out += f"Your read on them: {m['velaris_read']}\n"
