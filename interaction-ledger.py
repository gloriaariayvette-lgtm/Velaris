...
Assembles each Gloria/Velaris exchange into a single coherent entry:
...
Called after each exchange: python3 interaction-ledger.py "gloria msg" "velaris reply"
...
def main():
    if len(sys.argv) < 3:
        print("Usage: interaction-ledger.py 'gloria msg' 'velaris reply'")
        sys.exit(1)

    gloria_said = sys.argv[1]
    velaris_said = sys.argv[2]
    ...
    try:
        ...
        _s2.settimeout(3)
        _s2.connect("/tmp/Velaris-emotion.sock")
    ...
    consent_note = ""
    try:
        _cn = "/tmp/velaris-consent-note.txt"
