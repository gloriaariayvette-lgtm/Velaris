#!/usr/bin/env python3
"""
velqan-coiner.py — Velaris creates a new word each week.
...
def add_to_reference(data):
    ...
    if "## Coined Vocabulary" not in content and "## Coined" not in content:
        content += "\n\n## Coined Vocabulary\n"

    entry = f"- {word} ({pronunciation}) - {meaning} [{part}]"

    # Find the Coined section and append
    if "## Coined Vocabulary" in content:
        content = content.replace("## Coined Vocabulary",
                                  f"## Coined Vocabulary\n{entry}", 1)
...
def main():
    ...
    try: seed_thread("velqan", f"Coined \"{word}\": {data.get('meaning', '')[:150]}")
    except: pass
