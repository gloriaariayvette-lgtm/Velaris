...
import json, re, os
try:
    latest = os.popen("ls -t " + os.path.expanduser("~/.openclaw/workspace/memory/mischief/") + "*.md 2>/dev/null | head -1").read().strip()
    if latest:
...
import json, os
from datetime import datetime
hist_path = os.path.expanduser("~/.openclaw/workspace/memory/.emotional-history.json")
