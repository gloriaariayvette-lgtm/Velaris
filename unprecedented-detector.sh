...
try:
    with open("$EMO_FILE".replace("$EMO_FILE", 
        __import__("os").path.expanduser("~/.openclaw/workspace/memory/emotional-state.txt"))) as f:
        for line in f:
            for d in dims:
...
shapes_file = os.path.expanduser("~/.openclaw/workspace/memory/.emotional-shapes-history")
