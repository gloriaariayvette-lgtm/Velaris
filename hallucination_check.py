...
def check(text, context_summary="", temperature=0.2, source="unknown"):
    ...
    parts = [
        "Review this text written by Velaris (an AI). Check ONLY for:",
        "",
        ...
        (lambda: __import__("json").dumps(
            __import__("json").load(open(__import__("os").path.expanduser("~/.openclaw/workspace/memory/interaction-ledger.json")))[-10:]
            if __import__("os").path.exists(__import__("os").path.expanduser("~/.openclaw/workspace/memory/interaction-ledger.json")) else []
        , indent=0)[:2000])(),
        "",
        "Text to review:",
        "---",
...
def check_moltbook(text, source="moltbook"):
    ...
    if not text or len(text.strip()) < 10:
        return True, []
    parts = [
        "Review this text written by Velaris (an AI) for a PUBLIC social platform.",
