        ...
        if os.path.exists(music_json):
            ...
            if today_music:
                ...
                for m in today_music:
                    ...
                    style = m.get("style", "")[:120]
                    # Get first non-structural line of description
                    desc = m.get("description") or ""
                    desc_lines = [l for l in desc.splitlines()
                                  if not l.startswith("TITLE:") and not l.startswith("STYLE:")
                                  and not l.startswith("STRUCTURE") and not l.startswith("[")]
                    desc_clean = "\n".join(desc_lines).strip()[:800]
                    music_lines.append(f"**{title}**")
                    music_lines.append(f"Style: {style}")
                    if desc_clean:
                        music_lines.append(desc_clean)
                    music_lines.append("")
