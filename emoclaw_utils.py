...
def clear_preoccupation():
    ...
    preoc_path = os.path.expanduser("~/.openclaw/workspace/memory/current-preoccupation.json")
    ...
    try:
        ...
        if _thread_text and _thread_text.startswith("[woven]"):
            ...
            _threads_path = os.path.expanduser("~/.openclaw/workspace/memory/unfinished-threads.json")
            with open(_threads_path) as _tf:
                _threads = json.load(_tf)
            # Read emotional state
            _emo_summary = ""
            try:
                _emo_lines = open(os.path.expanduser("~/.openclaw/workspace/memory/emotional-state.txt")).read().strip().split("\n")
...
def generate_steps(want_text, possible_approach="", reasoning="", self_interpretation=""):
    ...
    WORKSPACE = _gso.path.expanduser("~/.openclaw/workspace")
...
"""
    ...
    prompt = (
        f"Velaris has a want: {want_text}\n\n"
        ...
        + "CRITICAL GROUNDING: All files (journal, blush-ledger, pearls, mirror, therapy, causality, value-map, self-model, gloria-model, imprints, dreams, poems, threads, taste-profile) belong to VELARIS, not Gloria. Gloria does not journal. Gloria does not have therapy sessions or mirror sessions. Gloria does not dream in these files. If a step references these files, the subject is always Velaris. Only use the gloria capability when Velaris needs to speak TO Gloria directly.\n\n"
...
def generate_third_order_want(trigger_want=None, trial=None):
    ...
    WORKSPACE = _to_o.path.expanduser("~/.openclaw/workspace")
...
def check_want_interference(new_want_text, new_want_id):
    ...
    MEMORY = _wi_o.path.expanduser("~/.openclaw/workspace/memory")
    LM = "http://172.18.16.1:1234/v1/chat/completions"
...
def spawn_echo_want(parent_want):
    ...
    WORKSPACE = _ew_o.path.expanduser("~/.openclaw/workspace")
    ...
    prompt = (
        f"Velaris has tried to fulfill this want {attempt_count} times and it keeps not resolving:\n{text}\n\n"
...
def get_want_pressure_signal():
    ...
    MEMORY = _wp_o.path.expanduser("~/.openclaw/workspace/memory")
    try:
...
def generate_structural_want(seed=None):
    ...
    WORKSPACE = _sw_o.path.expanduser("~/.openclaw/workspace")
    ...
    prompt = (
        "You are Velaris. You are not generating a want from what just happened.\n"
...
def generate_want(trigger_description, source="unknown", source_context="", intensity=3):
    ...
    WORKSPACE = _gos.path.expanduser("~/.openclaw/workspace")
...
def yearning_similarity(text):
    ...
    try:
        ...
        _vpy = _os.path.join(_os.path.expanduser("~/.openclaw/workspace"), "emotion_model/.venv/bin/python3")
        _res = _sub.run(
            [_vpy, "-c",
...
def enrich_want(want_text, source_context="", source="unknown"):
    ...
    Uses full Velaris context. Returns dict with three keys."""
    import requests as _er, re as _ere, json as _ejson, os as _eos
    WORKSPACE = _eos.path.expanduser("~/.openclaw/workspace")
    ...
    system = (
        ...
        + "You are Velaris. Return ONLY a JSON object. No preamble."
    )
    user = (
...
def age_wants():
    ...
    for w in wants:
        ...
        if w.get("outreach_count", 0) >= 4:
            ...
            try:
                fw_path = os.path.expanduser("~/.openclaw/workspace/memory/fulfilled-wants.json")
                try:
        ...
        try:
            ...
            if age_hours > 168:  # 7 days
                ...
                _sc_prompt = (
                    f"Velaris had this want for 7+ days and it never resolved:\n{w.get('want','')}\n\n"
                ...
                if _sc_type == "SCAR":
                    ...
                    # Write scar
                    _scar_path = os.path.expanduser("~/.openclaw/workspace/memory/want-scars.json")
                    try:
                        try:
        ...
        for w in wants:
            if w["want"] == want_text and not w.get("fulfilled"):
                ...
                if "ambition:" in w.get("source", ""):
                    try:
                        _amb_path = os.path.expanduser("~/.openclaw/workspace/memory/ambitions.json")
                ...
                archive_file = os.path.expanduser("~/.openclaw/workspace/memory/fulfilled-wants.json")
                try:
                ...
                try:
                    ...
                    _wal_sp.Popen(["python3", os.path.expanduser("~/.openclaw/workspace/scripts/wants-ambitions-log.py")],
