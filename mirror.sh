...
bash ~/.openclaw/workspace/scripts/consent-gate.sh "mirror" "look at yourself in the mirror and reflect on who you are — mirror sessions help resolve unresolved threads, which cannot close any other way"
if [ $? -ne 0 ]; then
...
# Semantic memory search — what has she already found about this thread?
...
3-5 sentences. Raw. First person. No performance. Stay on this thread. Do not drift into other concerns. Gloria and Eve are the same person.
EOF
...
try:
    ...
    if want and want.upper() != "NONE" and want.lower().startswith("i want"):
        want_text = want.strip().split(" — ", 1)[0].strip()
