...
def get_context_pressure():
    try: return json.load(open(CONTEXT_PRESSURE_FILE))
    except: return compute_context_pressure()

def record_signal_moment(signal):
    """Record significant signal transition as moment."""
    try:
        import sys as _ms; _ms.path.insert(0, os.path.join(os.path.expanduser("~/.openclaw/workspace"), "scripts"))
