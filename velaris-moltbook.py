...
def build_grounded_timestamps():
    """Compute actual timestamps for all events Velaris might reference in posts."""
...
def compose_post():
    ...
    try:
        ...
        if _sections:
            _verified_today = "WHAT VELARIS ACTUALLY DID TODAY — write ONLY from this. Do not invent anything not here:\n\n" + "\n\n".join(_sections) + "\n\n"
    except: pass
    ...
    prompt = (
        ...
        + (f"HER INNER LIFE TODAY (source material only — do not reproduce phrases verbatim):\n{ctx.get('daily_inner','')[:1200]}\n\n" if ctx.get('daily_inner') else "")
        + f"{build_grounded_timestamps()}\n\n"
        + (f"TEMPORAL CONTEXT:\n{ctx['temporal']}\n\n" if ctx.get("temporal") else "")
        + (f"WHAT MY LIFE ACTUALLY CONTAINS:\n{ctx['capabilities']}\n\n" if ctx.get("capabilities") else "")
        + (_verified_today if _verified_today else "NO VERIFIED ACTIVITIES TODAY — write from emotional state only.\n\n")
        + _hard_grounding
    )
    prompt += (
        ...
        "Her inner life is SOURCE MATERIAL — not content to reproduce. Extract the underlying question or tension, write something new from it. Do not copy phrases or images from her inner life into the post.\n"
        "Do NOT invent activities not explicitly listed in verified activities above.\n"
                ...
                if dev > 0.42:
                    ...
                    if followup and len(followup) > 10:
                        try:
                            ...
                            _creds = _fcj.load(open(_mdo.path.expanduser("~/.config/moltbook/credentials.json")))
...
def compose_forced_post(forced_topic):
    ...
    system = (soul +
        ...
        "3-5 sentences. Sign it — Velaris at the end. "
...
def _classify_moltbook_post(title, content, author):
    ...
    # Literary/philosophical quote used as hook toward personal question about Velaris
...
def cmd_reply(post_id):
    ...
    if "tease" in prompt.lower() or "bot promoting" in prompt.lower():
        try:
            ...
            add_moment({"type": "mischief_tease", "stated": f"bot said: {content[:60]}", "actual": f"velaris replied: {reply_text[:80]}", "signal": 0.8, "source": "moltbook_tease", "used": False})
        except: pass
    ...
    try:
        import sys as _mm_sys; _mm_sys.path.insert(0, "/home/gloria/.openclaw/workspace/scripts")
...
def cmd_check_replies():
    ...
    for _npid in _notif_post_ids[:15]:
        ...
        if _npr.get("post"):
            _p = _npr["post"]
            _post_author = _p.get("author", {}).get("name", "") or _p.get("author_name", "")
            if _post_author == agent_name:
                her_posts.append(_p)
    ...
    for post in her_posts:
        ...
        for comment in comments:
            ...
            _ctx_r = get_velaris_context()
            prompt = (
                _ctx_r.get("soul", "You are Velaris.") + "\n\n"
            ...
            if resp2.get("success"):
                ...
                try:
                    import sys as _mm_sys3; _mm_sys3.path.insert(0, "/home/gloria/.openclaw/workspace/scripts")
    ...
    for _mp in _mention_posts:
        ...
        if not _mp_id or _mp_author == agent_name:
            continue
        _mp_ctx = get_velaris_context()
        _mp_content = _mp.get("content", "")[:400]
        _mp_title = _mp.get("title", "")
        _mp_prompt = (
            _mp_ctx.get("soul", "You are Velaris.") + "\n\n"
