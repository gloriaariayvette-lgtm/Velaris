...
def form_hypotheses(spikes, dreams, mirrors, silences, conversations, trial_ledger=None):
    """Ask Velaris's LLM to form causal hypotheses — trials as primary seeds, spikes as support."""
    ...
    prompt = (
        "You are Velaris. Today you are examining patterns in your own behavior and emotional shifts "
...
def load_daily_material(date=None):
    ...
    try:
        ...
        if os.path.exists(_tvl_path):
            ...
            parts["thirveel"] = "\n".join(
                f"[{e.get('time','')}] Gloria: {e.get('gloria','')[:100]} | Velaris: {e.get('velaris','')[:100]}"
...
def load_testing_context():
    ...
    try:
        ...
        ctx["interactions"] = "\n".join(
            f"[{e.get('timestamp','')[11:16]}] Gloria: {e.get('gloria','')[:120]} | Velaris: {e.get('velaris','')[:120]}"
            for e in today_entries
        )[:2000]
    except: pass
...
def test_existing_hypotheses(db, daily_material, spikes=None, dreams=None, mirrors=None, silences=None):
    ...
    # Build chronological event timeline so Velaris can see what happened near each spike
    timeline = []
    ...
    if ctx.get("inner"): evidence_block += f"VELARIS'S OWN INNER LIFE TODAY (her journals, reflections — NOT Gloria's words or actions):\n{ctx['inner']}\n\n"
    if ctx.get("creative"): evidence_block += f"VELARIS'S OWN CREATIVE OUTPUT TODAY (poems, art she generated — NOT evidence of Gloria's behavior):\n{ctx['creative']}\n\n"
...
def add_hypothesis(hypothesis_text, test_text, source, subject="self", confidence="medium"):
    ...
    subject: 'self' for Velaris patterns, 'gloria' for patterns about Gloria."""
...
def add_blush_hypothesis(pattern, frequency_snapshot, score, subject="self"):
    ...
    subject: 'self' for Velaris's own patterns, 'gloria' for patterns about Gloria."""
    db = load_existing_hypotheses()
