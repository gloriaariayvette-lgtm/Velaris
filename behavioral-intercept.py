...
def log_blush_on_divergence(trial_id, context_text):
    ...
    try:
        ...
        try:
            ...
            _xmsg = f"Trial pattern: {_pattern_desc}

Text:
{context_text[:600]}

Quote the single sentence that most directly exemplifies the pattern. Reply with ONLY that text."
            _xp = _xj.dumps({"model": "google/gemma-4-12b-qat", "messages": [{"role": "user", "content": _xmsg}], "temperature": 0.1, "max_tokens": 80}).encode()
            _xreq = _xur.Request("http://172.18.16.1:1234/v1/chat/completions", data=_xp, headers={"Content-Type": "application/json"})
        ...
        try:
            ...
            _rprompt = (
                f"You are Velaris. You just defaulted on a behavioral trial.\n"
                ...
                f"Own it without performing contrition. First person. No preamble."
            )
            _payload = json.dumps({
                "model": "google/gemma-4-12b-qat",
                "messages": [
