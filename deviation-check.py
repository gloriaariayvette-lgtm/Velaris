...
def embed(text):
    ...
    r = requests.post(f"{LM_URL}/v1/embeddings",
        json={"model":"nomic-embed-text-v1.5","input":text[:500]},
...
def pick_discomfort_voice(dev_score, aln_score, violating_core=None, reason=""):
    """Generate his own voice expressing the felt discomfort or alignment."""
    if dev_score > DEVIATION_THRESHOLD:
        if reason:
            return reason[:120], "deviation"
...
def append_voice_coherence(voice_line, result_type, dev_score, aln_score, reason=""):
...
def check(reply_text, gloria_msg=""):
    ...
    try:
        _r = _req.post("http://172.18.16.1:1234/v1/chat/completions", json={
            ...
            "messages":[
                {"role":"system","content":f"You are evaluating a response against behavioral patterns. Answer with JSON only: {{\"deviation\": 0.0-1.0, \"alignment\": 0.0-1.0, \"reason\": \"one sentence naming the specific phrase or move that caused the score\"}}\n\nPatterns to check:\n{patterns_text}\n\ndeviation = how much the response exhibits the avoidance patterns\nalignment = how much the response moves toward the 'toward' behaviors"},
        ...
        if _m:
            ...
            _deviation_reason = _scores.get("reason", "")
            if pairs:
    ...
    result = "neutral"
    try: _pick_reason = _deviation_reason
    except: _pick_reason = ""
    voice_line, result_type = pick_discomfort_voice(final_dev, final_aln, violating_core, reason=_pick_reason)
    ...
    if final_dev > DEVIATION_THRESHOLD:
        ...
        # Write structured blush on Core deviation
        try:
            ...
            print(f"[Deviation] blush write failed: {_ble}")

        # Tag for causality — deviation_tags.json
        try:
            import json as _etj, os as _eto, datetime as _etdt
            _ef = _eto.path.join(MEMORY, "deviation_tags.json")
            _edb = _etj.load(open(_ef)) if _eto.path.exists(_ef) else {"tags": []}
            if "tags" not in _edb: _edb = {"tags": []}
            _now = _etdt.datetime.now()
            _edb["tags"].append({
                "id": f"dev_{_now.strftime('%Y%m%d_%H%M%S')}",
                "timestamp": _now.isoformat(),
                "date": _now.date().isoformat(),
                "category": "core_deviation",
                "pattern": _core_name,
                "score": round(final_dev, 3),
                "note": f"Something pulled toward {_core_name} here — worth noticing, not worth punishing. The pull itself is information.",
                "what_was_said": reply_text[:200] if reply_text else "",
                "reason": _deviation_reason if _deviation_reason else "",
                "source": "deviation_check"
            })
            _edb["tags"] = _edb["tags"][-200:]
            _etj.dump(_edb, open(_ef, "w"), indent=2)
            print(f"[Deviation] Tagged: {_core_name} ({final_dev:.2f})")
        except Exception as _ete:
            print(f"[Deviation] Tag failed: {_ete}")
        ...
        try: _vc_reason = _deviation_reason
        except: _vc_reason = ""
        append_voice_coherence(voice_line, "deviation", final_dev, final_aln, reason=_vc_reason)
                    ...
                    if e["name"] == aligning_core["name"]:
                        ...
                        if e["reinforcement_count"] >= 4 and e.get("polarity") == "positive":
                            try:
                                ...
                                _ci_sys.path.insert(0, SCRIPTS)
                                from causal_self_model import promote_to_commitment_imprint as _ci_promote
                                _ci_promote(
                                    e.get("almost_becoming", e.get("name","")),
