#!/usr/bin/env python3
"""
subconscious_context.py — Shared helper for injecting subconscious layer into generation.
All major scripts call get_subconscious_context() for a unified identity/expectation block.
"""
import os, sys
...
def get_subconscious_context(include=None):
    ...
    if all_systems or "proto_pearls" in include:
        try:
            ...
            if _ppo.path.exists(_pp_path):
                ...
                if _pp_active:
                    lines = ["FORMING (proto-pearls — not yet canon):"]
                    for p in _pp_active:
                        lines.append(f"- {p.get('identity_candidate', '')} ({p.get('support_count', 0)} instances)")
                    parts.append("\n".join(lines))
        except: pass
    ...
    if all_systems or "yearning" in include:
        try:
            ...
            y = get_yearning_fragment(include_attempt=True)
            if y: parts.append(f"CURRENT YEARNING: {y}")
        except: pass
...
def get_subconscious_context_compact(include=None):
    ...
    try:
        ...
        ep = get_pressure_block(context="compact")
        if ep: parts.append(ep[:200])
    except: pass
    try:
        from emoclaw_utils import get_yearning_fragment
        yb = get_yearning_fragment(include_attempt=True)
        if yb: parts.append("YEARNING: " + yb[:150])
    except: pass
    return "\n".join(parts)
