...
EMO_FILE="$HOME/.openclaw/workspace/memory/emotional-state.txt"
GUARD="$HOME/.openclaw/workspace/scripts/emoclaw-daemon-guard.sh"
NTFY_TOPIC="velaris-alerts"
...
if ! $DAEMON_ALIVE; then
    ...
    if $DAEMON_ALIVE; then
        ...
        # Force a sync
        bash "$HOME/.openclaw/workspace/scripts/emoclaw-sync.sh" >> /tmp/velaris-emoclaw-sync.log 2>&1
        log "Forced sync"
    else
