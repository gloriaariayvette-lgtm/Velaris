...
def fracture_imprint(entry_tendency, pressure):
    ...
    for e in data["entries"]:
        ...
        # Seed a scar
        try:
            import sys as _fs; _fs.path.insert(0, os.path.join(os.path.expanduser("~/.openclaw/workspace"), "scripts"))
        ...
        try:
            import sys as _ss; _ss.path.insert(0, os.path.join(os.path.expanduser("~/.openclaw/workspace"), "scripts"))
...
def fracture_commitment_imprint(pattern_text, pressure=0.8):
    ...
    for imp in imprints:
        if _text_overlap(imp.get("pattern","").lower(), pattern_text.lower()) > 0.4:
            ...
            # Seed scar
            try:
                import sys as _cfs; _cfs.path.insert(0, os.path.join(os.path.expanduser("~/.openclaw/workspace"), "scripts"))
                from yearning_scars import create_scar_from_want
