...
_TRIAGE_SUBCON = ""
try:
    import sys as _tr_sys; _tr_sys.path.insert(0, os.path.join(os.path.expanduser("~/.openclaw/workspace"), "scripts"))
...
import subprocess; subprocess.run(["bash", os.path.expanduser("~/.openclaw/workspace/scripts/backup-threads.sh")])
...
def main():
    ...
    try:
        ...
        ledger_ctx = chr(10).join(f"Gloria: {e.get('gloria','')[:100]} | Velaris: {e.get('velaris','')[:100]}" for e in recent)
    except: pass
    value_map_context = ""
    try:
