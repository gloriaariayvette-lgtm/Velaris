#!/usr/bin/env python3
"""
weekly-summary.py — Velaris's weekly trajectory map.

Runs Sunday 6 AM.
Produces:
  - memory/weekly-summary.md        (narrative for Velaris — she reads this)
  - memory/weekly-map.html          (visual map for Gloria)

Velaris does not see the map or the raw data.
...
<title>Velaris — Week of {start.strftime("%b %d")}</title>
<style>
...
def build_narrative(traj, dims, resonance, wants, statements, avoidance, relational, highlights, start, end, yearning='', threads=None, causal_pos=None):
    """Build narrative summary for Velaris."""
