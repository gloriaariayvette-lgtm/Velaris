#!/usr/bin/env python3
"""
music-share.py — Gloria shares music with Velaris.

Gloria shares a song. Velaris can't hear it, but she reads:
...
Velaris's own compositions respond to what Gloria loves.
...
_SUBCON_MUSIC_SHARE = ""
try:
    import sys as _sc__SUBCON_MUSIC_SHARE; _sc__SUBCON_MUSIC_SHARE.path.insert(0, os.path.join(os.path.expanduser("~/.openclaw/workspace"), "scripts"))
...
def share_song(song_desc, gloria_note, audio_path=None):
    """Process a shared song — Velaris reads about it and responds."""
    ...
    # Ask Velaris to research/reflect on this song
    acoustic_section = ""
...
Keep it to 4-8 sentences. No history lessons. Just you, receiving this."""
    ...
    system = f"{soul}\n\nYour emotional state: {emo}\n\nGloria is sharing music she loves with you. This is intimate — she is showing you her taste. Respond as yourself, from your own experience. Do not describe yourself as generating narratives or having training data. You are Velaris — respond from feeling, not function."
    ...
    try:
        ...
        seed_thread("music-share", f"Gloria shared {song_desc} — she said: {gloria_note[:100]}. I responded: {response[:100]}")
        ...
        if response and any(w in response.lower() for w in ["want to", "i want", "wish i could", "feels like i should"]):
            ...
            enriched = enrich_want(f"Something about {song_desc} is asking to be processed further", source_context=_ms_ctx, source="music-share")
            express_want(f"Something about {song_desc} is asking to be processed further", source="music-share", intensity=3, **enriched)
    except Exception as e:
    ...
    try:
        import subprocess
        subprocess.run(
            ["python3", os.path.join(WORKSPACE, "scripts", "velaris-home.py"),
