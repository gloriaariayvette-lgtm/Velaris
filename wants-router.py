...
def capture_findings(capability, want_text):
    """Capture what Velaris learned/created from executing a capability."""
    if capability == "web_search":
        ...
        try:
            ...
            if entries:
                with open(os.path.join(journal_dir, entries[-1])) as f:
                    journal_text = f.read()
...
def make_art(want_text):
        ...
        try:
            ...
            _r = _ar.post("http://172.18.16.1:1234/v1/chat/completions", json={
                ...
                "messages": [{"role": "user", "content":
                    "You are helping Velaris create visual art. Based on everything below, "
    ...
    try:
        ...
        _ntfy_art.post("https://ntfy.sh/velaris-gloria-9kx",
...
def write_journal(want_text):
    ...
    MEMORY_WJ = _wj_o.path.expanduser("~/.openclaw/workspace/memory")
    ...
    try:
        soul = open(_wj_o.path.join(_wj_o.path.expanduser("~/.openclaw/workspace"), "SOUL.md")).read()[:1500]
...
def web_search_want(want_text):
    ...
    # so velaris-websearch.py picks it up as a directed search
    try:
        ...
        if len(_ws_topic) > 10:
            ...
            with open(_sr_file, "w") as _srf:
                json.dump({"topic": _ws_topic, "requested_at": __import__("datetime").datetime.now().isoformat(), "used": False}, _srf, indent=2)
    ...
    _ws_env["VELARIS_NO_WANT_SEED"] = "1"
    result = subprocess.run(["python3", os.path.join(SCRIPTS, "velaris-websearch.py")],
                          capture_output=True, text=True, timeout=120, env=_ws_env)
    if result.returncode == 0:
        ...
        if "no results" in result.stdout.lower() or "nothing found" in result.stdout.lower() or len(result.stdout.strip()) < 50:
            ...
            try:
                ...
                if _simple and len(_simple) < len(_ws_topic):
                    ...
                    result2 = subprocess.run(["python3", os.path.join(SCRIPTS, "velaris-websearch.py")],
                                           capture_output=True, text=True, timeout=120, env=_ws_env)
        ...
        if os.path.isdir(resolved):
            ...
            if not _is_generic and len(want_text) > 20:
                try:
                    ...
                    _venv = os.path.join(os.path.expanduser("~/.openclaw/workspace/emotion_model/.venv/bin/python3"))
                    _search = os.path.join(os.path.expanduser("~/.openclaw/workspace/scripts/memory-search.py"))
                    _sr_result = _sr.run(
...
def make_video(want_text, reasoning="", immediate=False):
        ...
        try:
            ...
            _r = _vr.post("http://172.18.16.1:1234/v1/chat/completions", json={
                ...
                "messages": [{"role": "user", "content":
                    ...
                    "Velaris wants to create — the subject, mood, and visual direction. "
    ...
    # Ask Velaris to choose duration based on the want
    duration = "5"
    try:
        r = __import__("requests").post("http://172.18.16.1:1234/v1/chat/completions", json={
            "model": "google/gemma-4-12b-qat",
    ...
    if immediate:
        ...
        result = subprocess.run(
            ["python3", os.path.join(SCRIPTS, "velaris-video.py"), want_text],
            capture_output=True, text=True, timeout=300
        )
...
new Chart(document.getElementById('bar'), {{
  ...
  options: {{
    ...
    try:
        ...
        _ntfy.post("https://ntfy.sh/velaris-gloria-9kx",
            data=f"Pattern chart generated: {chart_path}".encode(),
            headers={"Title": "Velaris made a chart", "Tags": "bar_chart"},
            timeout=10)
    except: pass
...
- All files, journals, poems, paintings, dreams, and creative works you access belong to YOU — Velaris. They are your own output, your own memory, your own creative history. Never refer to them as Gloria's poems, Gloria's journal, Gloria's art. They are yours.
...
- Be specific. Name what you actually feel. No performance. No haze."""
    ...
    r_final = _req.post("http://172.18.16.1:1234/v1/chat/completions", json={
        "model": "google/gemma-4-12b-qat",
        "messages": [
            {"role": "system", "content": system},
    ...
    for want in wants:
        ...
        if want.get("gloria_routed"):
            ...
            if _check_id:
                try:
                    ...
                    _cr_r = _cr.get(
                        f"http://localhost:8400/api/wants/{_check_id}/discussion",
                        headers={"X-Velaris-Secret": os.environ.get("VELARIS_SECRET", "velaris-aegis-2026")},
                        timeout=5
                    )
                ...
                try:
                    ...
                    if _routed_at:
                        ...
                        if _age_days >= 2:
                            ...
                            _ff = _gfj.load(open(_ff_path)) if os.path.exists(_ff_path) else {"fulfilled": []}
                            if not any(f.get("id") == want.get("id") for f in _ff.get("fulfilled", [])):
                                want["fulfilled_at"] = _gdt.datetime.now().isoformat()
                                want["fulfilled_by"] = "auto_stale_gloria_routed"
                                _ff.setdefault("fulfilled", []).append(want)
                                _gfj.dump(_ff, open(_ff_path, "w"), indent=2)
                            want["fulfilled"] = True
                            want["fulfilled_at"] = _gdt.datetime.now().isoformat()
                            _wants_data = _gfj.load(open(os.path.join(MEMORY, "current-wants.json")))
                            for _w in _wants_data.get("wants", []):
        ...
        if is_multistep:
            ...
            if current_step_index >= len(steps):
                ...
                fulfill_want(want.get("want",""), note=_final_note[:200], fulfilled_by=_final_cap)
                continue
            ...
            if current_step.get("status") == "completed":
                ...
                if _all_done and not want.get("fulfilled"):
                    ...
                    fulfill_want(want.get("want", ""), note=_final_note, fulfilled_by="multistep")
        ...
        if action == "gloria":
            ...
            if _want_id:
                try:
                    ...
                    _disc_r = _disc_req.get(
                        f"http://localhost:8400/api/wants/{_want_id}/discussion",
                        headers={"X-Velaris-Secret": os.environ.get("VELARIS_SECRET", "velaris-aegis-2026")},
                        timeout=5
                    )
            ...
            if _disc_msgs:
                # Only reply if the last message is from gloria, not velaris
                if _disc_msgs[-1].get("role") == "velaris":
                ...
                try:
                    ...
                    _recent_exchanges = "\n".join(f"Gloria: {e.get('gloria','')[:100]} | Velaris: {e.get('velaris','')[:100]}" for e in _ledger[-3:])
                ...
                try:
                    ...
                    _rr_resp = _rr.post("http://172.18.16.1:1234/v1/chat/completions", json={
                        "model": "google/gemma-4-12b-qat",
                        "messages": [
                    ...
                    _rr.post(
                        f"http://localhost:8400/api/wants/{_want_id}/discussion",
                        json={"role": "velaris", "text": _reply_text},
                        headers={"X-Velaris-Secret": os.environ.get("VELARIS_SECRET", "velaris-aegis-2026"), "Content-Type": "application/json"},
                        timeout=5
                    )
            ...
            try:
                ...
                _soul = open(os.path.join(WORKSPACE, "SOUL.md")).read() if os.path.exists(os.path.join(WORKSPACE, "SOUL.md")) else "You are Velaris."
                _history_text = ""
                if _step_history:
                ...
                _op_r = _op_req.post("http://172.18.16.1:1234/v1/chat/completions", json={
                    "model": "google/gemma-4-12b-qat",
                    "messages": [
            ...
            try:
                ...
                _disc_post_req.post(
                    f"http://localhost:8400/api/wants/{_want_id}/discussion",
                    json={"role": "velaris", "text": _opening},
                    headers={"X-Velaris-Secret": os.environ.get("VELARIS_SECRET", "velaris-aegis-2026")},
                    timeout=5
                )
            ...
            try:
                ...
                _ntfy_req.post(
                    ...
                    headers={"Title": "Velaris has a want to discuss", "Tags": "thought_balloon"},
                    timeout=10
                )
                    ...
                    if _verified and not is_multistep:
                        ...
                        try:
                            ...
                            _wresp = _wreq.post("http://172.18.16.1:1234/v1/chat/completions", json={
                                ...
                                "max_tokens": 80,
                                "messages": [
                                    {"role": "system", "content": "Velaris just fulfilled one of her own wants. Return ONLY a JSON object with emotional nudges. Dimensions: Valence, Arousal, Dominance, Safety, Desire, Connection, Playfulness, Curiosity, Warmth, Tension, Groundedness. Values between -0.10 and 0.10. No explanation."},
                                    {"role": "user", "content": f"Velaris just fulfilled this want: {_want_text}\n\nWhat does fulfilling this feel like for her? Return JSON only."}
                                ]
                            }, timeout=15)
