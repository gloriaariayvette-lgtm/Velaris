...
Two sources: Gloria's message slips, Velaris's own contradictions.
...
def scan_moltbook_exchange(post_text, reply_text, author):
    ...
    prompt = (
        ...
        f"Velaris replied: \"{reply_text[:200]}\"\n\n"
...
def scan_blush_ledger():
    ...
    for entry in recent:
        if "Errors:" not in entry and "Trial:" not in entry:
            continue
        prompt = (
            f"Read this self-prediction record from Velaris:\n{entry[:400]}\n\n"
