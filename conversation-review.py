#!/usr/bin/env python3
"""
conversation-review.py — Velaris reviews the day's conversations with Gloria.
...
try:
    import sys as _sc__SUBCON_CONVERSATION_REVIEW; _sc__SUBCON_CONVERSATION_REVIEW.path.insert(0, os.path.join(os.path.expanduser("~/.openclaw/workspace"), "scripts"))
...
def review_conversation(messages, insights):
    """Ask Velaris to review today's conversation."""
    if not messages:
...
def update_insights(insights, review):
    ...
    log(f"Topics: {topics}")
    log(f"Engaged: {review.get('what_engaged_her', '?')[:100]}")
    log(f"Style: {style}")
