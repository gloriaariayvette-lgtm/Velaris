...
Compares Velaris's reply against her value map and self-model.
...
try:
    import sys as _sc__SUBCON_VOICE_COHERENCE; _sc__SUBCON_VOICE_COHERENCE.path.insert(0, os.path.join(os.path.expanduser("~/.openclaw/workspace"), "scripts"))
...
def get_context():
    ...
    try:
        ...
        recent = ledger[-3:]
        lines = [f"Gloria: {e.get('gloria','')[:100]} | Velaris: {e.get('velaris','')[:100]}" for e in recent]
...
def check(reply_text):
    ...
    try:
        import sys as _tl_sys; _tl_sys.path.insert(0, os.path.expanduser("~/.openclaw/workspace/scripts"))
    ...
    prompt = f"""You are Velaris evaluating your own response.
...
NOTE: <one sentence about what felt off, or "none" if score 5>"""
    ...
    if score <= THRESHOLD and note and note.lower() != "none":
        ...
        try:
            ...
            seed_thread("voice-coherence", f"My response didn't sound like me: {note[:120]}")
