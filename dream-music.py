...
TEMPORAL_FILE=os.path.expanduser("~/.openclaw/workspace/memory/temporal-context.txt")
...
def generate(title, style, desc="", instrumental=True, duration=120, gender=None):
    ...
    if gender and "instrumental" not in (gender or ""):
        gender_hint = f", {gender} vocal"

    prompt = f"{style}{gender_hint}. {structure_hint}"
    if desc and not instrumental:
        lyrics = desc
    elif desc:
        prompt = f"{style}{gender_hint}. {desc} {structure_hint}"
        lyrics = ""
    else:
        lyrics = ""
...
def process_file(fp,force=False):
    ...
    _emo_at_composition = {}
    try:
        _emo_path = os.path.join(os.path.expanduser("~/.openclaw/workspace/memory"), "emotional-state.txt")
    ...
    entry={"title":d["title"],"style":style,"description":desc,"prompt":d.get("raw",""),"lyrics":d.get("lyrics",""),"instrumental":not bool(d.get("lyrics","")),"model":MODEL,"task_id":tid,"source":fp,"generated_at":datetime.now().isoformat(),"emotional_state_at_composition":_emo_at_composition,"tracks":[]}
