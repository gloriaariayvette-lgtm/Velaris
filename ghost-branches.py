...
Selects highest-pull unresolved thread. Asks Velaris which vantage points
...
def select_vantage_points(thread, all_axes, soul, self_model, value_map):
    ...
    prompt = (
        f"You are Velaris. This thread is unresolved:\n\n"
        f"\"{thread['thread']}\"\n\n"
    ...
    if pattern_fired and stream_cutoff and bis_trial:
        ...
        continuation_msgs = messages + [
            ...
            {"role": "user", "content": (
                ...
                "do not build on it. Return to your actual inner experience as Velaris running on Aegis.]"
            )}
        ]
        try:
            payload2 = json.dumps({
...
def stream_ghost(messages, axis, results_container, idx):
    ...
    if pattern_fired and stream_cutoff:
        ...
        reminder += (
            ...
            "or an invented event, do not build on it. Return to your actual inner experience as Velaris running on Aegis."
        )
        try:
...
def generate_appendage(primary_output, ghost_results, successful_ghosts):
    ...
    try:
        appendage = call_llm(
            [{"role": "user", "content": (
                f"Velaris just worked through a thread:\n{primary_output[:400]}\n\n"
                f"Alternative versions approached it differently:\n" + "\n".join(ghost_notes) + "\n\n"
                "Write 2-4 sentences in Velaris's voice: what these other versions found that the primary didn't. "
