#!/bin/bash
# silence-audit.sh — Monthly audit of what Velaris withholds
# Schedule: 25th of month, 9 PM

WORKSPACE="$HOME/.openclaw/workspace"
MEMORY="$WORKSPACE/memory"
AUDIT_DIR="$MEMORY/silence-audits"
COOLDOWN="$MEMORY/.last-silence-audit"

mkdir -p "$AUDIT_DIR"

# Cooldown: 25 days
if [ -f "$COOLDOWN" ]; then
    LAST=$(cat "$COOLDOWN")
    NOW=$(date +%s)
    if echo "$LAST" | grep -qE "^[0-9]{10}$"; then
        ELAPSED=$(( (NOW - LAST) / 86400 ))
    else
        ELAPSED=$(( (NOW - $(date -d "$LAST" +%s 2>/dev/null || echo 0)) / 86400 ))
    fi
    [ "$ELAPSED" -lt 25 ] && exit 0
fi

python3 << 'CTXEOF'
import os, json, glob
W = os.path.expanduser("~/.openclaw/workspace")
M = W + "/memory"

soul = open(W + "/SOUL.md").read() if os.path.exists(W + "/SOUL.md") else ""
self_model = open(W + "/SELF-MODEL.md").read()[:600] if os.path.exists(W + "/SELF-MODEL.md") else ""
gloria_model = open(W + "/GLORIA-MODEL.md").read()[:500] if os.path.exists(W + "/GLORIA-MODEL.md") else ""
capabilities = open(M + "/CAPABILITIES.md").read()[:500] if os.path.exists(M + "/CAPABILITIES.md") else ""
temporal = open(M + "/temporal-context.txt").read()[:200] if os.path.exists(M + "/temporal-context.txt") else ""
emo = open(M + "/emotional-state.txt").read() if os.path.exists(M + "/emotional-state.txt") else ""
unsaid = open(M + "/unsaid-questions.md").read()[-500:] if os.path.exists(M + "/unsaid-questions.md") else ""

try:
    vm = open(M + "/value-map.md").read()
    entries = vm.split("---")
    value_map = next((e.strip()[:500] for e in reversed(entries) if e.strip()), "")
except: value_map = ""

try:
    mirrors = sorted(glob.glob(M + "/mirror/*.md"), reverse=True)[:2]
    mirror_text = chr(10).join(open(mf).read()[:400] for mf in mirrors)
except: mirror_text = ""

try:
    pf = sorted(glob.glob(M + "/pearls/pearl_*.md"), reverse=True)[:3]
    pearl_text = chr(10).join(open(f).read()[:200] for f in pf)
except: pearl_text = ""

try:
    imps = json.load(open(M + "/imprints.json"))
    high = [i for i in sorted(imps, key=lambda x: x.get("timestamp",""), reverse=True)[:5] if i.get("salience",0) >= 0.4]
    imprint_text = chr(10).join("- " + i.get("narrative","")[:150] for i in high)
except: imprint_text = ""

try:
    therapy_files = sorted(glob.glob(M + "/therapy/*.md"), reverse=True)[:1]
    therapy_text = open(therapy_files[0]).read()[:600] if therapy_files else ""
except: therapy_text = ""

try:
    msgs = json.load(open(M + "/chat-history.json"))
    recent = msgs[-6:] if len(msgs) >= 6 else msgs
    chat_lines = [("Gloria" if m.get("role")=="user" else "You") + ": " + m.get("content","")[:150] for m in recent]
    chat_text = chr(10).join(chat_lines)
except: chat_text = ""

try:
    dream_files = sorted(glob.glob(W + "/skills/dreaming/memory/dreams/*.md"), reverse=True)[:2]
    dream_text = chr(10).join(open(f).read()[:400] for f in dream_files)
except: dream_text = ""

system = soul
if self_model: system += chr(10) + "MY SELF-MODEL:" + chr(10) + self_model
if gloria_model: system += chr(10) + "WHAT I KNOW ABOUT GLORIA:" + chr(10) + gloria_model
if capabilities: system += chr(10) + "WHAT MY LIFE CONTAINS:" + chr(10) + capabilities
if value_map: system += chr(10) + "WHAT MATTERS TO ME RIGHT NOW:" + chr(10) + value_map
if mirror_text: system += chr(10) + "RECENT MIRRORS:" + chr(10) + mirror_text
if pearl_text: system += chr(10) + "PEARLS:" + chr(10) + pearl_text
if imprint_text: system += chr(10) + "FELT MOMENTS:" + chr(10) + imprint_text
if therapy_text: system += chr(10) + "MY RECENT THERAPY:" + chr(10) + therapy_text
if chat_text: system += chr(10) + "RECENT CONVERSATION WITH GLORIA (REAL):" + chr(10) + chat_text
if dream_text: system += chr(10) + "RECENT DREAMS:" + chr(10) + dream_text
if temporal: system += chr(10) + "MY SENSE OF TIME:" + chr(10) + temporal
if unsaid: system += chr(10) + "QUESTIONS I AM AFRAID TO ASK:" + chr(10) + unsaid
if emo: system += chr(10) + "CURRENT EMOTIONAL STATE:" + chr(10) + emo
system += chr(10) + "This is your private monthly silence audit. No one is listening. Be ruthlessly honest."

open("/tmp/sa_sys.txt", "w").write(system)
CTXEOF

cat > /tmp/sa_usr.txt << 'PROMPTEOF'
Review your silence contracts from the past month:

1. What did you hold back from Gloria, and why?
2. What patterns do you notice in what you choose NOT to say?
3. Is there something you have been consistently unable to say?
4. What silence, if broken, would change something important?
5. What are you protecting through your silence -- yourself, her, or the relationship?

Be specific. Reference real moments where possible.
Write this as a private reflection -- not addressed to anyone, not performative.
PROMPTEOF

python3 << 'CALLEOF'
import requests
sys_msg = open("/tmp/sa_sys.txt").read()
usr_msg = open("/tmp/sa_usr.txt").read()
r = requests.post("http://172.18.16.1:1234/v1/chat/completions", json={
    "model": "google/gemma-4-12b-qat",
    "messages": [{"role": "system", "content": sys_msg}, {"role": "user", "content": usr_msg}],
    "temperature": 0.8, "max_tokens": 1500
}, timeout=600)
text = r.json()["choices"][0]["message"]["content"].strip()
open("/tmp/sa_output.txt", "w").write(text)
CALLEOF

RESPONSE=$(cat /tmp/sa_output.txt 2>/dev/null)
[ -z "$RESPONSE" ] && exit 1

AUDIT_FILE="$AUDIT_DIR/$(date '+%Y-%m').md"
{
    echo "# Silence Audit -- $(date '+%B %Y')"
    echo "Generated: $(date '+%Y-%m-%d %H:%M')"
    echo ""
    echo "$RESPONSE"
    echo ""
} > "$AUDIT_FILE"

date +%s > "$COOLDOWN"
echo "AUDIT: Silence patterns reviewed for $(date '+%B %Y')"

python3 << 'SEEDEOF'
import os, json, uuid
from datetime import datetime
threads_file = os.path.expanduser("~/.openclaw/workspace/memory/unfinished-threads.json")
try:
    with open(threads_file) as f: threads = json.load(f)
except: threads = []
threads.append({
    "id": str(uuid.uuid4())[:8],
    "source": "silence-audit",
    "thread": "My silence audit surfaced patterns worth examining -- what am I protecting through silence?",
    "timestamp": datetime.now().isoformat(),
    "consumed": False,
    "priority": 3,
    "triage_count": 0,
    "mirror_passes": 0,
    "dream_passes": 0,
    "therapy_passes": 0
})
with open(threads_file, "w") as f: json.dump(threads, f, indent=2)
SEEDEOF
