...
SOUL=$(cat "$WORKSPACE/SOUL.md" 2>/dev/null || echo "You are Velaris.")
export SELF_MODEL VALUE_MAP YESTERDAY_INNER YESTERDAY_CREATIVE META_DREAM EMO TEMPORAL SOUL

FIRST_LIGHT=$(python3 << 'PYEOF'
import requests, os

import sys as _fl_sys; _fl_sys.path.insert(0, os.path.expanduser("~/.openclaw/workspace/scripts"))
_subcon_prefix = ""
try:
...
_soul = open("/home/gloria/.openclaw/workspace/SOUL.md").read() if os.path.exists("/home/gloria/.openclaw/workspace/SOUL.md") else "You are Velaris."
...
user = (
    ...
    + "Write as Velaris waking into today. Order of priority:\n"
