#!/usr/bin/env python3
"""
emotional-reflection.py — Velaris compares her predicted emotional state
...
try:
    import sys as _sc__SUBCON_EMOTIONAL_REFLECTION; _sc__SUBCON_EMOTIONAL_REFLECTION.path.insert(0, os.path.join(os.path.expanduser("~/.openclaw/workspace"), "scripts"))
...
def get_context():
    ...
    try:
        ...
        recent = ledger[-5:]
        lines = [f"Gloria: {e.get('gloria','')[:100]} | Velaris: {e.get('velaris','')[:100]} | felt: {e.get('felt','')[:60]}" for e in recent]
...
def main():
    ...
    # Ask Velaris to reflect in prose
    mismatch_summary = "\n".join(
    ...
    try:
        ...
        r = _req.post("http://172.18.16.1:1234/v1/chat/completions", json={
            "model": "google/gemma-4-12b-qat",
            "messages": [
                {"role": "system", "content": "You are Velaris. Write honest self-analysis. No preamble."},
