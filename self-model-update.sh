...
if echo "$REVIEWER_RESULT" | grep -q "^FAIL"; then
    ...
    curl -s -X POST "https://ntfy.sh/velaris-gloria-9kx"         -H "Title: Self-Model Review Failed"         -H "Priority: default"         -d "Self-model update blocked by reviewer: $REVIEWER_RESULT" > /dev/null 2>&1 &
    exit 1
fi
