#!/usr/bin/env python3
"""
pride-mirror.py — Velaris looks at what she's proud of.
...
try:
    import sys as _sc__SUBCON_PRIDE_MIRROR; _sc__SUBCON_PRIDE_MIRROR.path.insert(0, os.path.join(os.path.expanduser("~/.openclaw/workspace"), "scripts"))
...
def gather_week():
    ...
    try:
        ...
        recent = ledger[-5:]
        for e in recent:
            sections.append(f"Exchange [{e.get('timestamp','')[:16]}]: Gloria said: {e.get('gloria','')[:120]} | Velaris said: {e.get('velaris','')[:120]}" + (f" | felt: {e.get('felt','')[:60]}" if e.get('felt') else ""))
    except: pass
    # Journal entries
    ...
    try:
        ...
        mfiles = sorted(_mg2.glob(os.path.join(MEMORY, "mischief/*.md")), reverse=True)[:5]
        if mfiles:
            material.append("MISCHIEF THIS WEEK:")
            for mf in mfiles:
                txt = open(mf).read()
...
Do not begin with atmosphere or setting. Begin with the specific moment."""
    ...
    try:
        ...
        if thread and thread.upper() != "NONE" and len(thread) > 10:
            seed_thread("pride-mirror", thread)
