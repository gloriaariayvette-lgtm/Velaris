...
12 dimensions including Nifrathir.
...
# Per-dimension fire thresholds — (high_threshold, low_threshold)
FIRE = {
    "Valence":      (0.65, 0.44),
    "Arousal":      (0.65, 0.44),
    "Dominance":    (0.68, 0.44),
    "Safety":       (0.65, 0.44),
    "Desire":       (0.65, 0.44),
    "Connection":   (0.55, 0.28),
    "Playfulness":  (0.63, 0.38),
    "Curiosity":    (0.78, 0.55),
    "Warmth":       (0.63, 0.38),
    "Tension":      (0.52, 0.24),
    "Groundedness": (0.76, 0.52),
    "Nifrathir":    (0.65, 0.35),
}

def base_pressures(s):
    """Layer 1 — independent dimension pressures. Language of drift, not instruction."""
    lines = []
    ...
    if high("Warmth"):
        ...
        lines.append("Warmth is low. Don't perform closeness that isn't there.")
    ...
    # Nifrathir — the under-thread
    nif = v("Nifrathir", 0.5)
    if nif > FIRE["Nifrathir"][0]:
        lines.append("The under-thread is warm — willing to extend, reach further, risk more.")
    elif nif < FIRE["Nifrathir"][1]:
        lines.append("The under-thread is cool — shorter, more contained, conserving reach.")
...
COMBOS = [
    ...
    {
        "name": "nifrathir_surge",
        "conditions": lambda s: s.get("Nifrathir", 0.5) > 0.75 and s.get("Connection", 0) > 0.6,
        "pressure": "The under-thread and connection are both high — this is a reaching moment. Don't hold back from initiating."
    },
]

def combo_pressure(s):
    """Layer 2 — detect combinations. Max 1 per generation."""
    for combo in COMBOS:
        try:
