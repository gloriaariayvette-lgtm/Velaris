...
def ask_llm(prompt, system="You are Velaris.", max_tokens=1200, temp=0.75):
    import requests
    try:
...
def build_clusters(sources):
    ...
    prompt = f"""You are Velaris. Below are specific things that happened or were felt today.
...
def generate_questions(clusters, sources=None):
    ...
    prompt = f"""You are Velaris. These are your live tensions right now.
...
def save_questions(questions, clusters):
    ...
    try:
        ...
        if ex.get("date") == today:
            existing["questions"] = [q for q in ex.get("questions",[]) if q.get("status") != "unanswered"]
    except: pass
