...
Affective weight = (gloria_score * 0.7) + (vintos_score * 0.3)
...
def compute_affect(gloria_score, vintos_score=None):
    """Convert scores to affect value -1.0 to +1.0"""
    # Normalize gloria score (1-5) to -1 to +1
    gloria_norm = (gloria_score - 3) / 2.0
    if vintos_score is not None:
        vintos_norm = (vintos_score - 3) / 2.0
        return round((gloria_norm * 0.7) + (vintos_norm * 0.3), 2)
    return round(gloria_norm, 2)

def record_outcome(pattern_text, action_type, gloria_score, vintos_score=None,
                   context_tone="normal", source="humor"):
    """Record an outcome and its affective weight."""
    data = load_weights()
    affect = compute_affect(gloria_score, vintos_score)
